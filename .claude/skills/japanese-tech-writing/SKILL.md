...
description: 日本語で技術文書（書籍の章、記事、解説文）を執筆・推敲するときに使う。
metadata:
  language: ja
...
原稿で定義した術語（本質的複雑さ、回収、判断の配置など）を議論に使うのはよい。空虚な装飾として使うのが問題である。
...
- 「この文書もそれを否定しない」のような、著者の立場の弁明や断りを書かない。事実の記述（「〜に書かせる場合が多い」）だけを置く。
